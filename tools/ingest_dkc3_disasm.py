...
# Routines the static recompiler must not materialize, kept on the runtime's
# interpreter tier with `force_lle`. Each entry names why; the reasons are
# structural facts of the game's code, not tuning.
FORCE_LLE = {
    # The number printer at $80:A65F pops its own return address, pushes the
    # digits it divides out, re-pushes the address, and returns; its caller
    # at $80:A64A then pops those digits off the stack. A recompiled caller
    # and an interpreted callee disagree about that stack, and the caller's
    # return lands at address zero (the lockup after the first file select).
    0x80A64A: "pops digits its callee left on the stack",
